...
public class ArrayListStudentTests {
    ...
    private int getExpectedLength(int i) {
        int expectedLength = ArrayListInterface.INITIAL_CAPACITY;
        if (i > 0) {
            expectedLength = ArrayListInterface.INITIAL_CAPACITY *
    ...
    public void test_11_constructor() {
        assertEquals("A new empty ArrayList should have a backing array of size " +
                ArrayListInterface.INITIAL_CAPACITY + ".",
                ArrayListInterface.INITIAL_CAPACITY, list.getBackingArray().length);
        assertEquals("A new empty ArrayList should be size 0.", 0, list.size());
    }

    @Test(timeout = TIMEOUT)
    public void test_12_addToFront() {
        for (int i = 1; i <= ArrayListInterface.INITIAL_CAPACITY + 1; i++) {
    ...
    public void test_14_addAtIndex() {
        ...
        for (int i = 1; i <= ArrayListInterface.INITIAL_CAPACITY; i++) {
            list.addToBack("" + i);
        }
        list.addAtIndex(0, "" + 0);
        ...
        for (int i = 0; i < ArrayListInterface.INITIAL_CAPACITY; i++) {
            list.addToBack("" + i);
        }
